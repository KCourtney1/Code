...
class Lexer:
    ...
    def __init__(self, code: str) -> None:
        ...
        self.token_specs = [
            ("HEXNUMBER", r"0x[0-9a-fA-F]+"),   #Hex 
            ("BOOL", r"True|False"),            #Boolean
            ("TYPE", r"\b(int|hex|bool)\b"),    #variable types
            ("ENDIF", r"endif"),                #endif keyword
            ("ENDFOR", r"endfor"),              #endfor keyword
            ("IF", r"if"),
            ("ELSE", r"else"),
            ("FOR", r"for"),
            ("TO", r"to"),
...
class Parser:
    ...
    def parse_statement(self) -> ASTNode:
        ...
        if token_kind == "IDENTIFIER":
            ...
            raise SyntaxError(f"Unexpected token {token_kind}")
    ...
    def parse_decl_stmt(self) -> Declaration:
        ...
        """
        token_type = self.expect('TYPE')
        var_type = token_type[1]

        identifier = self.expect('IDENTIFIER')
        var_name = identifier[1]

        if not self.checkVarRedeclared(var_name): #checking if redeclared in current scope
            self.add_variable(var_name, var_type)
        return Declaration(token_type, identifier)
    ...
    def parse_assign_stmt(self) -> Assignment:
        ...
        self.expect("EQUALS")
        expr_node, expr_type = self.parse_expression()

        if self.checkVarDeclared(var_name): #checking if declared before use
            var_type = self.get_variable_type(var_name)
            if var_type != expr_type and expr_type != 'error_type':   #rule 4 and prevents double error if expr_type is already invalid
                self.checkAssignErrorMsg(var_name, var_type, expr_type)
        return Assignment(identifier, expr_node)
    ...
    def parse_if_stmt(self) -> IfStatement:
        ...
        """
        self.expect("IF")
        condition, condition_type = self.parse_boolean_expression()

        if condition_type != "bool" and condition_type != 'error_type': #rule 5 and prevent cascading errors
    ...
    def parse_for_stmt(self) -> ForStatement:
        ...
        iterator_name = iterator[1]

        self.checkVarDeclared(iterator_name)   #check if variable is declared

        self.expect("EQUALS")
        start_expr, start_type = self.parse_expression()

        self.expect("TO")
        end_expr, end_type = self.parse_expression()

        if self.checkVarDeclared(iterator_name):    #check type for interator
            iterator_type = self.get_variable_type(iterator_name)
            if iterator_type != start_type and start_type != 'error_type' : #rule 4 (start type) and prevents cascading errors
                self.checkAssignErrorMsg(iterator_name, iterator_type, start_type)
            elif iterator_type != end_type and end_type != 'error_type':    #rule 4 (end type) and prevents cascading errors
                self.checkAssignErrorMsg(iterator_name, iterator_type, end_type)
    ...
    def parse_boolean_expression(self) -> Tuple[ExprType, DataType]:
        ...
        """
        node, node_type = self.parse_boolean_term()
        while self.current_token()[0] == "OR":
            op = self.current_token()
            self.advance()
            right, right_type = self.parse_boolean_term()

            if node_type != 'bool' or right_type != 'bool':     #rule 1
                self.checkBoolOpErrorMsg()
    ...
    def parse_boolean_term(self) -> Tuple[ExprType, DataType]:
        ...
        """
        node, node_type = self.parse_boolean_factor()
        while self.current_token()[0] == "AND":
            op = self.current_token()
            self.advance()
            right, right_type = self.parse_boolean_factor()
    ...
    def parse_boolean_factor(self) -> Tuple[ExprType, DataType]:
        ...
        if token[0] == "NOT":
            self.advance()
            factor, factor_type = self.parse_boolean_factor()
            
            if factor_type == 'error_type':     #prevent casading errors
                return UnaryOperation(token, factor), 'bool'
            if factor_type != 'bool':   #rule 1
                self.checkBoolOpErrorMsg()
                
            return UnaryOperation(token, factor), factor_type
    ...
    def parse_comparison(self) -> Tuple[ExprType, DataType]:
        ...
        """
        node, node_type = self.parse_expression()
        while self.current_token()[0] in ("EQ", "NEQ", "LESS", "GREATER"):
            op = self.current_token()
            self.advance()
            right, right_type = self.parse_expression()

            if node_type != 'error_type' and right_type != 'error_type':    #checking if error already
                if node_type != right_type:                                 #rule 3
                    self.checkTypeMatchErrorMsg(node_type, right_type)
    ...
    def parse_expression(self) -> Tuple[ExprType, DataType]:
        ...
        """
        node, node_type = self.parse_term()
        while self.current_token()[0] in ("PLUS", "MINUS"):
            op = self.current_token()
            self.advance()
            right, right_type = self.parse_term()

            if node_type not in ('int', 'hex') or right_type not in ('int', 'hex'):     #rule 2
                self.checkArithOpErrorMsg()
                node_type = 'int'
            elif node_type != right_type:
                self.checkTypeMatchErrorMsg(node_type, right_type)
                node_type = 'int'
    ...
    def parse_term(self) -> Tuple[ExprType, DataType]:
        ...
        """
        node, node_type = self.parse_factor()
        while self.current_token()[0] in ("MULTIPLY", "DIVIDE", "MODULO"):
            op = self.current_token()
            self.advance()
            right, right_type = self.parse_factor()
    ...
    def parse_factor(self) -> Tuple[ExprType, DataType]:
        ...
        if token[0] in ("PLUS", "MINUS"):
            self.advance()
            factor, factor_type = self.parse_factor()

            if factor_type not in ('int', 'hex'):   #check valid types
                self.checkArithOpErrorMsg()
                return UnaryOperation(token, factor), 'int'
            return UnaryOperation(token, factor), factor_type
    ...
    def parse_primary(self) -> Tuple[ExprType, DataType]:
        ...
        if token[0] == "NUMBER":
            ...
            self.advance()
            var_name = token[1]
            if self.checkVarDeclared(var_name):     #check if declared
                return token, self.get_variable_type(var_name)
            return token, 'error_type'              #if not declared change type to error_type
        elif token[0] == "LPAREN":
            self.advance()
            node, node_type = self.parse_boolean_expression()
            self.expect("RPAREN")
            return node, node_type
        else:
    ...
    def exit_scope(self) -> None:
        if len(self.scope_stack) > 1:   #don't pop global
    ...
    def checkVarDeclared(self, identifier: str) -> None:
        ...
        """
        for scope_name in reversed(self.scope_stack):   #local "middle" global
