    ...
    def tokenize(self) -> List[Tuple[str, Any]]:
        ...
        for match in re.finditer(self.token_regex, self.code):
            ...
            if type == 'SKIP':
                ...
                if value == 'if':
                    self.tokens.append(('IF', value))
                elif value == 'else':
                    self.tokens.append(('ELSE', value))
                elif value == 'for':
                    self.tokens.append(('FOR', value))
                elif value == 'to':
                    self.tokens.append(('TO', value))
                elif value == 'print':
                    self.tokens.append(('PRINT', value))
                elif value == 'and':
                    self.tokens.append(('AND', value))
                elif value == 'or':
                    self.tokens.append(('OR', value))
                elif value == 'not':
                    self.tokens.append(('NOT', value))
                else:
                    self.tokens.append(('IDENTIFIER', value))
            elif type in ('IF', 'ELSE', 'FOR', 'TO', 'PRINT', 'AND', 'OR', 'NOT',
...
class Parser:
    ...
    def parse_statement(self) -> ASTNode:
        ...
        if token_type == 'IDENTIFIER':
            ...
            if next_token == "EQUALS":
                ...
                raise SyntaxError(f"Identifier '{self.current_token()[1]}' cannot start a statement at position {self.pos}")
        elif token_type == 'IF':
    ...
    def parse_for_stmt(self) -> ForStatement:
        ...
        """
        self.expect('FOR')
        identifier_value = self.expect('IDENTIFIER')
        
        self.expect('EQUALS')
        start = self.parse_boolean_expression()

        self.expect("TO")
        end = self.parse_boolean_expression()

        self.expect('COLON')
        block = self.parse_block()

        return ForStatement(identifier_value, start, end, block)
    ...
    def parse_block(self) -> Block:
        ...
        while self.current_token()[0] in ('IDENTIFIER', 'IF', 'FOR', 'PRINT'):
            statements.append(self.parse_statement())
    ...
    def parse_arg_list(self) -> List[ExprType]:
        ...
        args = [self.parse_boolean_expression()]
        while self.current_token()[0] == 'COMMA':
            self.expect('COMMA')
            args.append(self.parse_boolean_expression())
        return args
    ...
    def parse_boolean_expression(self) -> ExprType:
        ...
        while self.current_token()[0] == 'OR':
            op = self.expect('OR')
            RHS = self.parse_boolean_term()
            LHS = LogicalOperation(LHS, op, RHS)
        return LHS
    ...
    def parse_primary(self) -> ExprType:
        ...
        if token_type == 'LPAREN':
            ...
            raise SyntaxError(f"Unexpected primary token: {token_type} at position {self.pos}")