    ...
    def update_p(self):
        for n in range(self.N):  # time loop
            for j in range(self.M):  # space loop
                ...
                if ((0 <= x <= 0.5) and (x > t)) or ((0.5 <= x <= 1) and (t<=(1/(1+self.epsilon)*(x-0.5)+0.5))):
                    self.p[n,j] = self.IC
                else:
                    self.p[n,j] = self.BC 
...
class implicit(Exact):
    ...
    def get_coefficient_m_standard(self):
        v = np.ones(self.M)
        v[self.M//2 +1:] = (1 + self.epsilon)
        nu = v * (self.dt/self.dx)
...
def density_check(s, tolerance = 10**-3, n=0):
    ...
    if epsilon != 0:
        true_ATT = 0.5 + (n * s.dt) + ((1/(1 +  s.epsilon)) * (1 -  0.5))
    else:
        true_ATT = 1
...
def error(approx, exact, tvalues = [0, .2, .4, .6, .8, 1]):
    tval_errors = []
    t = np.linspace(0, 1, approx.N)
    
    for t_val in tvalues:
        n = np.argmin(np.abs(t - t_val))
        error = np.abs(approx.p[n, 1:] - exact.p[n, 1:])
...
BC = 0.47
IC = 0.0
gamma_values = [0.1] #, 0.25, 0.5, 0.75, .9
...
for gamma in gamma_values:
    for epsilon in epsilon_values:
        ...
        for i in range(5, 12):
            ...
            FI_error_table.append(density_check(filtered_implicit_s, n=0))
            I_error_table.append(density_check(implicit_s, n=0))
        ...
        density_error_table = pd.DataFrame(I_error_table, columns=["dt","True ATT.", "Tolerance ATT. Error", "Tolerance Time Level","Tolerance Est. ATT.",
        ...
        density_error_table = pd.DataFrame(FI_error_table, columns=["dt","True ATT.", "Tolerance ATT. Error", "Tolerance Time Level","Tolerance Est. ATT.",
                                                                "Max ATT. Error", "Max Time Level", "Max Est. ATT."])
...
f2_ax1.set_xlabel('ln(dx)', fontsize=12); f2_ax1.set_ylabel('ln(error)', fontsize=12); f2_ax1.set_title('log-log Error at t=0.2')
f2_ax2.set_xlabel('ln(dx)', fontsize=12); f2_ax2.set_ylabel('ln(error)', fontsize=12); f2_ax2.set_title('log-log Error at t=1')

for df in filtered_implicit_error_dataframes:
    df_gamma = df["gamma"].values[0]
    dx = np.log(df["dx"].values)
    t_2 = np.log(df["t=0.2"].values)
    t_1 = np.log(df["t=1"].values)
    f2_ax1.plot(dx, t_2, 'o-', label="gamma = {}".format(df_gamma))
...
f3_ax1.set_xlabel('dx', fontsize=12); f3_ax1.set_ylabel('error', fontsize=12); f3_ax1.set_title('Filtered Implicit Error at t=0.2')
...
for df in filtered_implicit_error_dataframes:
    ...
    t_2 = df["t=0.2"].values
    t_1 = df["t=1"].values
    f3_ax1.plot(dx, t_2, 'o-', label="gamma = {}".format(df_gamma))
