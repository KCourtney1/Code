...
def density_check(s, tolerance = 10**-3):
    ...
    if epsilon != 0:
        ...
        true_ATT = 1
    m_time_level = np.argmax(s.p[:, s.M//2])

    m_est_ATT = m_time_level * s.dt
    t_est_ATT = t_time_level *  s.dt
    t_error = abs(t_est_ATT - true_ATT)
    m_error = abs(m_est_ATT - true_ATT)
    return [s.dx, t_error, t_time_level, t_est_ATT, m_error, m_time_level, m_est_ATT]
...
for epsilon in epsilon_values:
    for gamma in gamma_values:
        ...
        for i in range(11, 12):
            ...
            filterUpwind_s.update_p(); Exact_s.update_p();   #Upwind_s.update_p()

            #dx_UW_error.append([dx] + error(Upwind_s, Exact_s))
            dx_FUW_error.append([dx] + error(filterUpwind_s, Exact_s))
            ATT_table.append(density_check(filterUpwind_s))

        here = [[epsilon] + error(filterUpwind_s, Exact_s, tvalues=[0.5 + (1/(1 + epsilon) * (1 - 0.5))])]
        table = pd.DataFrame(here, columns=["Epsilon", "Err. gamma={}".format(gamma)])
        table.to_csv("special Epsilon = {}, Gamma = {}.csv".format(epsilon, gamma))
        ...
        density_ATT_table = pd.DataFrame(ATT_table, columns=["dx", 
                                                                "Tol. ATT. Error", "Tol. Time Level","Tol. Est. ATT.",
                                                                "Max ATT. Error", "Max Time Level", "Max Est. ATT."])
        density_ATT_table.to_csv("ATT_table Epsilon={}, Gamma = {}.csv".format(epsilon, gamma))
