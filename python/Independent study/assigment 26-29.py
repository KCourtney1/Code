...
class Exact:
    def __init__(self, dx, BC, IC, gamma) -> None:
        self.gamma = gamma
        self.dx = dx
        self.BC = BC
        self.IC = IC
...
class filteredImplicit(implicit):
    def __init__(self, dx, BC, IC, epsilon) -> None:
        super().__init__(dx, BC, IC, epsilon)
...
BC = .47
IC = 0.0
gamma_values = [0, 0.25, 0.5, 0.75, .9]  #gamma values to test
...
for gamma in gamma_values:
    ...
    for i in range(6, 9):
        dx = (1/2)**(i)
        plots = [implicit_s, filtered_implicit_s, Exact_s] = [implicit(dx, BC, IC, gamma),
                                                                            filteredImplicit(dx, BC, IC, gamma), 
                                                                            Exact(dx, BC, IC, gamma)]
        implicit_s.update_p()
    ...
    with pd.ExcelWriter("implicit error table gamma = {}.xlsx".format(gamma), engine="openpyxl") as writer:
        ...
        density_error_table.to_excel(writer, sheet_name="Density Error Table")

with pd.ExcelWriter("Rate of Convergence table.xlsx", engine="openpyxl") as writer:
    for i in range(0, len(filtered_implicit_error_dataframes)):
        roc = rate_of_convergence(filtered_implicit_error_dataframes[i])
        roc_df = pd.DataFrame(roc, columns=["error"])
        roc_df.to_excel(writer, sheet_name="ROC for gamma = {}".format(gamma_values[i]))
