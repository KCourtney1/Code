...
class implicit(Exact):
    ...
    def get_coefficient_m(self):
        ...
        main_diag = np.diag(np.full(N, 1 + nu)) 
        sub_diag = np.diag(np.full(N - 1, -nu), k=-1)
    ...
    def update_p(self):
        ...
        for n in range(0, self.N - 1):            
            self.p[n + 1, :] = inv @ (self.get_p_hat(n + 1) + self.p[n, :])
...
class filteredImplicit(implicit):
    ...
    def get_coefficient_m(self):
        ...
        main_diag = np.diag(np.full(N, main_diag_value)) 

        sub_diag_value = -nu
        sub_diag = np.diag(np.full(N - 1, sub_diag_value), k=-1)
    ...
    def update_p(self):
        ...
        first_inv = np.linalg.inv(implicit.get_coefficient_m(self))
        inv = np.linalg.inv(self.coefficient_m)
        nu = self.dt/self.dx

        for n in range(0, self.N - 1):
            if n == 0:
                self.p[n + 1, :] = first_inv @ (self.get_p_hat(n + 1) + self.p[n, :])
            else:
                self.p[n + 1, :] = inv @ (
                    (nu * self.get_p_hat(n + 1))
                    + ((1 + ((2 * self.gamma) / (2 - self.gamma))) * self.p[n, :])
                    - ((self.gamma / (2 - self.gamma)) * self.p[n - 1, :])
                )
...
for i in range(10, 12):
    ...
    dx_filtered_upwind_error.append([dx] + error(filtered_implicit_s, Exact_s))
...
with pd.ExcelWriter("implicit solution error table.xlsx", engine="openpyxl") as writer:
    ...
    error_filtered_implicit_table = pd.DataFrame(dx_filtered_upwind_error, columns=["dx","t=0", "t=0.2", "t=0.4", "t=0.6", "t=0.8", "t=1"])
